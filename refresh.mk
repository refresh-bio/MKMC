### REFRESH group macros - v.1.0.21 (2025-10-24)
...
# Add zlib-ng in zlib-compatibile mode
define ADD_ZLIB_NG_AS_ZLIB
	$(info *** Adding zlib-ng in zlib-compatibile mode ***)
	$(eval ZLIB_DIR:=$(1))
	...
	$(eval zlib-ng: $(ZLIB_A))
	$(eval $(ZLIB_A) : ; \
		cd $(ZLIB_DIR) && cmake $(CMAKE_OSX_FIX) -DCMAKE_CXX_COMPILER=$(CXX) -DCMAKE_C_COMPILER=$(CC) -B build-g++/zlib-ng -S . -DZLIB_COMPAT=ON -DWITH_GZFILEOP=ON; cmake --build build-g++/zlib-ng --config Release)
endef
...
define ADD_KMC_LIB
	...
	$(eval LIBKMC_DIR:=$(1))
	$(eval LIBKMC_A_DIR:=$(1))
	$(eval LIBKMC_A:=$(1)/bin/libkmc_core.a)
	$(eval LIBRARY_FILES+=$(LIBKMC_A))
	$(eval LINKER_DIRS+=-L $(LIBKMC_A_DIR))
	$(eval PREBUILD_JOBS+=libkmc)

	$(eval libkmc: $(LIBKMC_A))
	$(eval $(LIBKMC_A): ; \
		cd $(LIBKMC_DIR) && $(MAKE) bin/libkmc_core.a)
endef
