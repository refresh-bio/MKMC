...
bool SamplesFileReader::parseLine(const std::string& line, uint32_t lineNo, bool& singleWordLine)
{
	...
	{
		std::cerr << "Error: Sample " << sampleName << " is given multiple times in " << mkmcParams.inputFileName << ".\n" << std::endl;
		return false;
	}
...
bool SamplesFileReader::readSamples(std::vector<Sample>& oSamples)
{
	...
	if (samples.empty())
	{
		std::cerr << "Error: No samples specified in a " << mkmcParams.inputFileName << " file.\n" << std::endl;
		return false;
	}
